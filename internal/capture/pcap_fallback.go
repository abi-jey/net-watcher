...
// DNSSniffer handles DNS packet capture and parsing using libpcap (non-Linux fallback)
...
// NewDNSSniffer creates a new DNS sniffer for specified interface using pcap
...
// Start begins packet capture using pcap
func (d *DNSSniffer) Start() error {
	if d.debug {
		fmt.Printf("Starting DNS capture on interface %s (pcap fallback)\n", d.ifaceName)
	}
...
// processPackets captures and processes packets using pcap
