...
func (db *DB) GetEvents(filter EventFilter) ([]NetworkEvent, error) {
	var events []NetworkEvent
	query := db.Model(&NetworkEvent{})
	...
	if filter.IP != "" {
		query = query.Where("src_ip = ? OR dst_ip = ?", filter.IP, filter.IP)
	}
	if filter.SrcIP != "" {
	...
	if filter.Port > 0 {
		query = query.Where("src_port = ? OR dst_port = ?", filter.Port, filter.Port)
	}
	if filter.Interface != "" {
		query = query.Where("interface = ?", filter.Interface)
	}
	if filter.Domain != "" {
		query = query.Where("dns_query LIKE ?", "%"+filter.Domain+"%")
	}
	if filter.SNI != "" {
		query = query.Where("tlssni LIKE ?", "%"+filter.SNI+"%")
	}
...
func (db *DB) GetStats(since time.Time) (*Stats, error) {
	...
	db.Model(&NetworkEvent{}).Where("timestamp >= ? AND event_type = ?", since, EventTCPStart).Count(&stats.TCPCount)
	db.Model(&NetworkEvent{}).Where("timestamp >= ? AND event_type = ?", since, EventUDPStart).Count(&stats.UDPCount)
	db.Model(&NetworkEvent{}).Where("timestamp >= ? AND event_type = ?", since, EventDNS).Count(&stats.DNSCount)
	db.Model(&NetworkEvent{}).Where("timestamp >= ? AND event_type = ?", since, EventTLSSNI).Count(&stats.TLSCount)
	db.Model(&NetworkEvent{}).Where("timestamp >= ? AND event_type = ?", since, EventICMP).Count(&stats.ICMPCount)

	// Unique hosts (destinations)
	db.Model(&NetworkEvent{}).Where("timestamp >= ? AND dst_ip != ''", since).Distinct("dst_ip").Count(&stats.UniqueHosts)

	// Unique domains
	db.Model(&NetworkEvent{}).Where("timestamp >= ? AND dns_query != ''", since).Distinct("dns_query").Count(&stats.UniqueDomains)

	// Oldest and newest
	var oldest, newest NetworkEvent
	db.Model(&NetworkEvent{}).Where("timestamp >= ?", since).Order("timestamp ASC").First(&oldest)
	db.Model(&NetworkEvent{}).Where("timestamp >= ?", since).Order("timestamp DESC").First(&newest)
	stats.OldestEvent = oldest.Timestamp
	stats.NewestEvent = newest.Timestamp
	...
	db.Model(&NetworkEvent{}).
		...
		Where("timestamp >= ? AND dns_query != ''", since).
		Group("dns_query").
		Order("count DESC").
		Limit(10).
		Scan(&topDomains)
	for _, d := range topDomains {
		stats.TopDomains = append(stats.TopDomains, TopItem{Name: d.DNSQuery, Count: d.Count})
	}
	...
	db.Model(&NetworkEvent{}).
		...
		Where("timestamp >= ? AND dst_ip != '' AND event_type IN (?, ?)", since, EventTCPStart, EventUDPStart).
		Group("dst_ip").
		Order("count DESC").
		Limit(10).
		Scan(&topDests)
	for _, d := range topDests {
		stats.TopDestinations = append(stats.TopDestinations, TopItem{Name: d.DstIP, Count: d.Count})
	}
	...
	db.Model(&NetworkEvent{}).
		Select("tlssni, COUNT(*) as count").
		Where("timestamp >= ? AND tlssni != ''", since).
		Group("tlssni").
		Order("count DESC").
		Limit(10).
		Scan(&topSNIs)
	for _, s := range topSNIs {
		stats.TopSNIs = append(stats.TopSNIs, TopItem{Name: s.TLSSNI, Count: s.Count})
	}

	return stats, nil
}

// GetTimelineData returns events grouped by hour for timeline visualization
func (db *DB) GetTimelineData(since time.Time) ([]TimelinePoint, error) {
	var points []TimelinePoint
	err := db.Model(&NetworkEvent{}).
